...
behavior_data_regex = re.compile(r"(\d+)\s+(contingency|trialThresh|totalPokes|totalRewards|ifDelay|"
...
def parse_ifDelay_events_for_legacy_statescriptlog(behavior_data, logger):
    ...
    for trial in trial_info:
        ...
        trial.insert(total_rewards_idx + 1, 
...
def parse_trial_and_block_data(behavior_data, block_ends, logger):
    ...
    for row in range(0, len(behavior_data), info_rows_per_trial):
        ...
        current_port_visit_counts = {port_num: trial_dict[f"countPokes{port_num}"]["value"] 
                                     for port_num in [1, 2, 3]}
        end_port = next((i for i in [1, 2, 3] if current_port_visit_counts[i] == port_visit_counts[i] + 1), None)
        if end_port is None:
            logger.error(f"No end port detected for trial: {trial_within_block} "
                         f"in block {current_block.get('block_num')}")
            raise Exception(f"Warning: No end port detected for trial: {trial_within_block} "
                            f"in block {current_block.get('block_num')}")
...
def parse_nosepoke_events(nosepoke_events, nosepoke_DIOs, logger, poke_time_threshold=1):
    ...
    if event_counts["poke_in"] != event_counts["poke_out"]:
        logger.error(f"Got {event_counts['poke_in']} poke_in events "
                     f"but {event_counts['poke_out']} poke_out events in the statescript!")
        raise Exception(f"Warning: {event_counts['poke_in']} poke_in events "
                        f"but {event_counts['poke_out']} poke_out events in the statescript!")
    ...
    logger.info(f"{len(DIO_nosepoke_df)} nosepokes from the DIOs: "
                 f"{DIO_nosepoke_df['port'].value_counts().to_dict()}")
    logger.info(f"{len(statescript_nosepoke_df)} nosepokes from the statescript: "
    ...
    for row in range(0, len(statescript_nosepoke_df) - 1, 2):
        ...
        if not (event1["event_name"] == "poke_in" 
                and event2["event_name"] == "poke_out" 
                and event1["port"] == event2["port"]):
            logger.error("Warning: Invalid nosepoke pair from statescript at "
                         f"timestamps {event1['timestamp']} and {event2['timestamp']}!")
            raise Exception("Warning: Invalid nosepoke pair from statescript at "
                            f"timestamps {event1['timestamp']} and {event2['timestamp']}!")
    ...
    for row in range(0, len(DIO_nosepoke_df) - 1, 2):
        ...
        if not (event1["event_name"] == "poke_in" 
                and event2["event_name"] == "poke_out" 
                and event1["port"] == event2["port"]):
            logger.error("Warning: Invalid nosepoke pair from DIOs at "
                         f"timestamps {event1['timestamp']} and {event2['timestamp']}!")
            raise Exception("Warning: Invalid nosepoke pair from DIOs at "
                            f"timestamps {event1['timestamp']} and {event2['timestamp']}!")
    ...
    if len(DIO_nosepoke_df) > len(statescript_nosepoke_df):
        logger.warning(f"Length mismatch: {len(DIO_nosepoke_df)} nosepokes from DIOs, " 
                      f"but only {len(statescript_nosepoke_df)} nosepokes from statescript.\n"
                      "The DIO may have more pokes because it keeps recording after "
                      "the statescript has been stopped (this is ok).")
    # The statescript should never have more pokes than the DIOs - break if this happens so we can figure out why.
        ...
        logger.error(f"Length mismatch: {len(statescript_nosepoke_df)} nosepokes from statescript "
                    f"but {len(DIO_nosepoke_df)} nosepokes from DIOs!")
        raise Exception(f"Length mismatch: {len(statescript_nosepoke_df)} nosepokes from statescript "
                        f"but {len(DIO_nosepoke_df)} nosepokes from DIOs!")
    ...
    merged_nosepokes = pd.merge(DIO_nosepoke_df, statescript_nosepoke_df, 
                                on=['event_name', 'port', 'index'], how='inner', suffixes=('_DIO', '_statescript'))
    ...
    merged_nosepokes_outer = pd.merge(DIO_nosepoke_df, statescript_nosepoke_df, 
                                      on=['event_name', 'port', 'index'], how='outer', 
    ...
    for row in range(0, len(merged_nosepokes) - 1, 2):
        ...
        if not (poke_in["event_name"] == "poke_in" 
                and poke_out["event_name"] == "poke_out" 
                and poke_in["port"] == poke_out["port"]):
            logger.warning(f"Invalid nosepoke pair at timestamps {poke_in['timestamp_DIO']} "
                           f"and {poke_out['timestamp_DIO']}!")
            raise Exception(f"Invalid nosepoke pair at timestamps {poke_in['timestamp_DIO']} "
                            f"and {poke_out['timestamp_DIO']}!")
...
def combine_nosepoke_and_trial_data(nosepoke_df, trial_df, session_end, logger):
    ...
    logger.info("Note that occasionally there are delays in statescript printing, "
    ...
    if len(nosepoke_df) != 2 * len(trial_df):
        if session_end is None:
            ...
            # If the last event before the session end is a poke_out, no adjustment needed!
                ...
                logger.error("event_name must be either 'poke_in' or 'poke_out'! "
            ...
            if len(nosepoke_df) != 2 * len(trial_df):
                logger.error(f"After removing nosepokes after the session end, " 
                                f"expected {2*len(trial_df)} nosepokes for {len(trial_df)} trials, "
                                f"got {len(nosepoke_df)}")
                raise Exception(f"After removing nosepokes after the session end, " 
                                f"expected {2*len(trial_df)} nosepokes for {len(trial_df)} trials, "
                                f"got {len(nosepoke_df)}")
            logger.debug(f"After removing nosepokes after the session end, got the expected "
    ...
    for i, trial_row in trial_df.iterrows():
        ...
        if abs(time_diff) > 5000:
            logger.warning(f"Trial {trial_row['trial_within_session']} (block {trial_row['block']} "
                           f"trial {trial_row['trial_within_block']}), port {trial_row["end_port"]}:")
            logger.warning(f"Poke in at statescript time {poke_in_row['timestamp_statescript']} "
                           f"may not match trial printed at {trial_row['statescript_reference_timestamp']} "
                           f"(Time diff = {time_diff/1000}s)")
        ...
        if not ((trial_row["end_port"] == poke_in_row["port"]) and (trial_row["end_port"] == poke_out_row["port"])):
            logger.error(f"Trial ending at port {trial_row['end_port']} does not match "
                f"poke in at port {poke_in_row['port']} and poke out at port {poke_out_row['port']}")
            raise Exception(f"Trial ending at port {trial_row['end_port']} does not match "
                f"poke in at port {poke_in_row['port']} and poke out at port {poke_out_row['port']}")
...
def combine_reward_and_trial_data(trial_df, reward_DIOs, logger):
    ...
    """
    logger.info("Checking that reward data from the statescript matches reward data from the DIOs, "
    ...
    for key, (data, timestamps) in reward_DIOs.items():
        for i in range(0, len(data), 2):
            ...
            if not (data[i] == 1 and data[i + 1] == 0):
                logger.error(f"Data mismatch at index {i} for key {key}: "
                            f"expected [1, 0], got [{data[i]}, {data[i + 1]}]")
                raise Exception(f"Data mismatch at index {i} for key {key}: "
                                f"expected [1, 0], got [{data[i]}, {data[i + 1]}]")
            ...
            if not (abs(timestamps[i] - timestamps[i+1])<1):
                logger.error("Expected timestamps to be within 1s, "
                             f"got pump_on_time {timestamps[i]}, pump_off_time {timestamps[i+1]}]")
                raise Exception("Expected timestamps to be within 1s, "
                                f"got pump_on_time {timestamps[i]}, pump_off_time {timestamps[i+1]}]")
    ...
    if not reward_pump_df.equals(pd.DataFrame(reward_pump_times)
                             .sort_values(by="pump_off_time")
    ...
    logger.debug("Check passed: Pump events end up in the same order regardless of "
                 "if we sort by pump_on_time or pump_off_time")
    ...
    if len(reward_pump_df) != len(rewarded_trial_df):
        logger.warning(f"Expected {len(rewarded_trial_df)} reward DIO events "
                    f"for {len(rewarded_trial_df)} rewarded trials, got {len(reward_pump_df)}")
    else:
        logger.debug(f"Check passed: got {len(reward_pump_df)} reward DIO events "
    ...
    for trial_row, DIO_times in zip(rewarded_trial_df.itertuples(index=True), reward_pump_df.itertuples(index=False)):
        ...
        if trial_row.end_port != DIO_times.port:
            logger.error(f"Mismatch: trial end_port {trial_row.end_port} does not match "
                        f"reward pump port {DIO_times.port}")
            raise Exception(f"Mismatch: trial end_port {trial_row.end_port} does not match "
                        f"reward pump port {DIO_times.port}")

        # Ensure the reward pump turns on within a second of the poke
        if abs(trial_row.poke_in_time - DIO_times.pump_on_time) > 1:
            logger.error(f"Pump on at time {DIO_times.pump_on_time} may not match nosepoke "
                         f"at {trial_row.poke_in_time}")
            raise Exception(f"Pump on at time {DIO_times.pump_on_time} may not match nosepoke "
                            f"at {trial_row.poke_in_time}")
...
def adjust_block_start_trials(trial_data, block_data, DIO_events, excel_data, logger):
    ...
    logger.info("Adjusting the block start trials based on barrier_shift DIO events (if they exist)"
        "or data from the experimental notes excel sheet")
    ...
    if "barrier_shift" in DIO_events:
        ...
        for barrier_shift_time in barrier_shift_times:
            ...
            logger.info(f"Barrier shift DIO pressed {barrier_shift_time_from_poke:.2f}s "
    ...
    if barrier_shift_trials_DIO is not None and barrier_shift_trials_excel is not None:
        ...
        if barrier_shift_trials_DIO != barrier_shift_trials_excel:
            logger.warning(f"Mismatch in barrier shift info between barrier_shift DIOs and data from excel sheet!\n"
            ...
            logger.info(f"Barrier_shift DIOs match data from excel sheet, "
    ...
    # Or if only excel, use that
        ...
        logger.error("No 'barrier_shift' DIO event or 'barrier shift trial ID' from excel found\n"
    ...
    logger.debug("statescript_end_timestamp is now N/A because statescript timestamps "
                 "no longer correspond to barrier changes")
    ...
    for _, row in new_block_data.iterrows():
        trials_in_block = (
            (trial_data['trial_within_session'] >= row['start_trial']) &  
...
def validate_poke_timestamps(trial_data, logger):
    ...
    logger.info("Checking that the DIO poke_in_time and poke_out_time matches the statescript "
        "poke_in_time and poke_out_time for each trial, after converting units.")
    ...
    logger.debug("It is expected for the timestamps to drift apart over the course of a session "
    ...
    # Check poke_in times
    for i, (DIO_poke, ss_poke, port) in enumerate(zip(DIO_poke_in_times, 
                                                statescript_poke_in_times, 
                                                trial_data["end_port"]), start=1):
        diff = abs(DIO_poke - ss_poke)
        if diff > error_tol_ms:
            # Log big differences at error level! These are definitely concerning
            logger.error(f"Trial {i}, port {port}: "
            ...
            logger.warning(f"Trial {i}, port {port}: "
                           f"DIO poke_in at {DIO_poke:.1f} and statescript poke_in at {ss_poke} "
            ...
            logger.debug(f"Trial {i}, port {port}: "
                         f"DIO poke_in at {DIO_poke:.1f} and statescript poke_in at {ss_poke} "
                         f"are {diff:.1f} ms apart. ")

    # Check poke_out times
    for i, (DIO_poke, ss_poke, port) in enumerate(zip(DIO_poke_out_times, 
                                                      statescript_poke_out_times,
                                                      trial_data["end_port"]), start=1):
        diff = abs(DIO_poke - ss_poke)
        if diff > error_tol_ms:
            # Log big differences at error level! These are definitely concerning
...
def parse_state_script_log(statescriptlog, DIO_events, excel_data_for_epoch, logger):
    ...
    nosepoke_DIOs = {key: value for key, value in DIO_events.items() 
                     if key in ['wellA_poke', 'wellB_poke', 'wellC_poke']}
    ...
    if session_type == "barrier change":
        trial_data, block_data = adjust_block_start_trials(trial_df, block_data, 
                                                           DIO_events, excel_data_for_epoch, logger)
    ...
    block_data = add_barrier_locations_to_block_data(block_data, excel_data_for_epoch, 
                                                     session_type, logger)
    ...
    for key, (data, timestamps) in behavioral_event_data.items():
        for i in range(len(data) - 1):
            if not ((data[i] == 1 and data[i + 1] == 0) or (data[i] == 0 and data[i + 1] == 1)):
                ...
                logger.warning(f"{key} has mismatched DIO {data[i], data[i+1]} "
                               f"at timestamps {timestamps[i], timestamps[i+1]}")
...
def add_block_and_trial_data_to_nwb(nwbfile: NWBFile, trial_data, block_data, logger, overwrite=False):
    ...
    if not overwrite and ("block" in nwbfile.intervals or "trials" in nwbfile.intervals):
        ...
        logger.error("Stopping. Run again with overwrite=True "
                    "if you wish to overwrite the original block and trials table.")
        print("Stopping. Run again with overwrite=True "
              "if you wish to overwrite the original block and trials table.")
        return
...
def add_hex_centroids_to_nwb(nwbfile: NWBFile, hex_centroids_file_path, logger):
    ...
    if not required_columns.issubset(actual_columns):
        logger.error(f"Expected {required_columns} columns in the hex centroids file, "
...
def add_behavioral_data_to_nwb(nwb_path, excel_path, 
    ...
    with NWBHDF5IO(nwb_path, mode="r+") as io:
        ...
        for idx, epoch in run_epochs.iterrows():
            ...
            DIO_events_in_epoch = {
                ...
                if (filtered := [(d, ts) for d, ts in zip(data, timestamps) 
                                 if epoch.start_time <= ts <= epoch.stop_time])
                # if (filtered := [(d, ts) for d, ts in zip(data, timestamps) 
            ...
            trial_data, block_data = parse_state_script_log(statescriptlog, DIO_events_in_epoch, 
                                                            excel_data_for_epoch, logger)
            ...
            if epoch.start_time > trial_data.loc[0, 'start_time']:
                logger.info(f"Setting start time of the first block/trial to epoch start time {epoch.start_time}, "
        ...
        if "nwb" in save_type:
            ...
            add_block_and_trial_data_to_nwb(nwbfile, trial_data_all_epochs, block_data_all_epochs, 
                                            logger, overwrite)
