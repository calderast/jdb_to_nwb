...
def WhittakerSmooth(data,binary_mask,lambda_):
    ...
    - fits a smooth background to the input data ('data') by minimizing the sum of squared differences between the data and the background.
    ...
    ID_matrix = eye(data_size,format='csc') # creates an identity matrix the size of data_size in compressed sparse column (csc) format
    diff_matrix = ID_matrix[1:]-ID_matrix[:-1] # numpy.diff() does not work with sparse matrix. This is a workaround.
    diagonal_matrix = diags(binary_mask,0,shape=(data_size,data_size)) # creates a diagonal matrix with the binary mask values on the diagonal
    A = csc_matrix(diagonal_matrix+(lambda_*diff_matrix.T*diff_matrix)) # represents the combination of the diagonal matrix and the smoothness penalty term Lambda_
...
def airPLS(data, lambda_=100, penalty_order=1, max_iterations=15):
    ...
    for i in range(1,max_iterations+1):
        baseline = WhittakerSmooth(data,weights,lambda_, penalty_order) # loop runs 'max_iterations' times to adjust the weights and fit the baseline
        delta = data - baseline # difference between data and baseline to calculate residuals, how much each data point deviates from the baseline. delta > 0 == peak. delta < 0 == background.
        sum_of_neg_deltas = np.abs(delta[delta<0].sum()) # how much data is below the baseline. 

        if (sum_of_neg_deltas<0.001*(abs(data)).sum() or i==max_iterations): # convergence check: if sum_of_neg_deltas is less than 0.001 of the total data, or if the maximum number of iterations is reached, the loop breaks.
            if (i==max_iterations): print('WARING max iteration reached!')
            break

        weights[delta>=0] = 0 # delta >= 0 means that this point is part of a peak, so its weight is set to 0 in order to ignore it
        weights[delta<0] = np.exp(i*np.abs(delta[delta<0])/sum_of_neg_deltas) # updates the weights for the negative deltas. Gives more weight to larger residuals using an exponential function.
        weights[0] = np.exp(i*(delta[delta<0]).max()/sum_of_neg_deltas) # updates the weights for the first and last data points to ensure edges of data are not ignored or underweighed. 
        weights[-1] = weights[0] 
    return baseline
